...
public:
  ...
  auto add_repeat_event(uint32_t duration, std::function<void()> func, std::function<void(float)> iter_func = {}) noexcept -> uint32_t;

  auto add_single_event(uint32_t duration, std::function<void()> func, std::function<void(float)> iter_func = {}) noexcept -> uint32_t;
  ...
  auto get_progress(uint32_t id) const noexcept -> float;

  void set_progress(uint32_t id, float progress) noexcept;
