...
void CopyEngine::copy(std::vector<BitmapView> const& bitmaps, std::vector<ImageHandle> const& image_handles) noexcept
{
  auto& slot = _slot_pool[_slot];
  assert(slot.is_idle());
  slot.upload_buffer.add_images(image_handles, bitmaps);
}
