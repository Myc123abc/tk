...
auto ImageManager::try_load(std::string_view path) noexcept -> std::expected<ImageHandle, ImageLoadError::Type>
{
  ...
  if (!_load_tasks.contains(filename))
  {
    auto task = g_thread_pool.submit([path = std::string(path)]
    {
      ...
      return LoadResult{ data, w, h, msg };
    });
...
void ImageManager::update() noexcept
{
  ...
  for (auto it = _load_tasks.begin(); it != _load_tasks.end();)
  {
    ...
    if (_load_tasks[path.data()].is_completed())
    {
      ...
      auto [data, w, h, msg] = task.take_result();
      if (!data)
      {
      ...
      // TODO: mipmap use when the image is smaller than origin resolution
...
void ImageManager::load(std::string_view path, uint width, uint height, void* data) noexcept
{
  assert(!_loaded_images.contains(path.data()));

  auto handle = create(width, height, ImageFormat::rgba8_unorm, ImageType::srv);
