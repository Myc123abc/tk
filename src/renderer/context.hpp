...
public:
  void set_cmd(Command* cmd) noexcept;
  ...
  void set_render_target(ImageHandle render_tareget_image, ImageHandle depth_stencil_image) noexcept;
...
void Context::set_graphics_constants(uint root_param_idx, T const& constants) noexcept
{
      ...
      memcmp(_graphics_constants.data(), &constants, size))
  {
    ...
    _cmd->get()->SetGraphicsRoot32BitConstants(root_param_idx, _graphics_constants.size() / 4, _graphics_constants.data(), 0);
  }
}
...
void Context::set_compute_constants(uint root_param_idx, T const& constants)
{
      ...
      memcmp(_compute_constants.data(), &constants, size))
  {
    ...
    _cmd->get()->SetComputeRoot32BitConstants(root_param_idx, _compute_constants.size() / 4, _compute_constants.data(), 0);
  }
}
