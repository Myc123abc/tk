...
void Window::init(int x, int y, uint32_t width, uint32_t height, ui::Backdrop const& backdrop) noexcept
{
  ...
  g_renderer.create_window_resource(_handle, real_width(), real_height());
...
void Window::init_blur_window(ui::Backdrop const& backdrop) noexcept
{
  ...
  g_renderer.show_blur_window(_handle, _blur_window);
}
...
auto Window::move_from_maximize() noexcept -> int2
{
  ...
  g_ui_ctx.get_window_context(_handle)->frame_data.init(real_width(), real_height());
...
void Window::resize(ResizeType type, int dx, int dy) noexcept
{
  ...
  if (!_resizing)
  {
    ...
    ctx->need_clear = true;
    ctx->frame_data.init(real_width(), real_height());
  }
}
