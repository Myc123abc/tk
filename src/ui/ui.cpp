...
void render()
{
  ...
  assert(ctx->engine && ctx->shape_properties.back().op == type::shape_op::mix);
  assert(ctx->engine);
...
void path_end(uint32_t color, uint32_t thickness)
{
  ...
  if (ctx->union_start)
    ctx->shape_properties.back().op = type::shape_op::min;
  else
